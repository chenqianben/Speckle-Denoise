...
class KPN(Model):
    def __init__(self, color=False, burst_length=8, blind_est=False, sep_conv=False, kernel_size=[5], 
        ...
        self.conv1 = Basic(64, channel_att=False, spatial_att=False, use_bias = self.use_bias)  
        self.avgpool1 = layers.AveragePooling2D(pool_size=(2, 2))
        self.conv2 = Basic(128, channel_att=False, spatial_att=False, use_bias = self.use_bias)
        self.avgpool2 = layers.AveragePooling2D(pool_size=(2, 2))
        self.conv3 = Basic(256, channel_att=False, spatial_att=False, use_bias = self.use_bias)
        self.avgpool3 = layers.AveragePooling2D(pool_size=(2, 2))
        self.conv4 = Basic(256, channel_att=False, spatial_att=False, use_bias = self.use_bias)
        
        # decoder
        self.up4 = layers.UpSampling2D(size=(2,2), interpolation='bilinear')
        self.conv5 = Basic(256, channel_att=channel_att, spatial_att=spatial_att, use_bias = self.use_bias)
        self.up5 = layers.UpSampling2D(size=(2,2), interpolation='bilinear')
        self.conv6 = Basic(128, channel_att=channel_att, spatial_att=spatial_att, use_bias = self.use_bias)
        self.up6 = layers.UpSampling2D(size=(2,2), interpolation='bilinear')
        self.conv7 = Basic(64, channel_att=channel_att, spatial_att=spatial_att, use_bias = self.use_bias)
        self.outc = Basic(out_channel, channel_att=channel_att, spatial_att=spatial_att, use_bias = self.use_bias)
    ...
    def call(self, data_with_est, data): 
        ...
        core = self.outc(conv7)    # (bs, h, w, color*filter_size*filter_size)
        
        pred_img, pred_img_i  = self.kernel_pred(data, core) # data是原始data, core是data_with_est由unet得到的dynamic filter
        return pred_img, pred_img_i, core                     # pred_img_i: (bs, N, w, h, color) 和 pred_img: (bs, w, h, color)
...
class LossBasicSymetric(Model):
    """
    Basic loss function.
    """
    def __init__(self, gradient_L1=True, gamma = 0.0):
        super(LossBasicSymetric, self).__init__()
        self.l1_loss = keras.losses.MeanAbsoluteError()
        self.l2_loss = keras.losses.MeanSquaredError()
        self.gradient = TensorGradient(gradient_L1)
        self.gamma = gamma

    def call(self, pred, ground_truth, core):
        #print(self.l2_loss(pred, ground_truth), self.l1_loss(self.gradient(pred), self.gradient(ground_truth)), self.gamma * self.symetric_loss(core))
        return self.l2_loss(pred, ground_truth) + self.l1_loss(self.gradient(pred), self.gradient(ground_truth)) + self.gamma * self.symetric_loss(core)
    
    def symetric_loss(self, core):
        loss = 0
        lossfunc = keras.losses.MeanSquaredError()
        for k,v in core.items():
            v = tf.reshape(v, [-1, k**2])
            v = tf.reduce_mean(v, axis=0, keepdims=False)
            v_s = tf.reverse(v, axis=[0])
            loss += lossfunc(v, v_s)
        return loss
