...
class GDFN(nn.Module):
    def __init__(self, filter_size = (3,3), color=False, blind_est=True, channel_att=False, spatial_att=False):
        super(GDFN, self).__init__()
        self.color_channel = 3 if color else 1
        self.color = 3 if color else 1
        in_channel = self.color * (1 if blind_est else 2)
        out_channel = self.color * self.color
        ...
        self.conv5 = Basic(256, 256, channel_att=False, spatial_att=False)
        
        self.outc1 = Basic(64+128+256+256+256, 128, channel_att=False, spatial_att=False)
        self.outc2 = Basic(128, 64, channel_att=False, spatial_att=False)
        self.outc3 = Basic(64, out_channel, channel_att=False, spatial_att=False)
        self.adapool = nn.AdaptiveAvgPool2d(filter_size)
    ...
    def forward(self, data_with_est, white_level=1.0):
        ...
        conv1d = F.avg_pool2d(conv1, kernel_size=16, stride=16)
        conv2d = F.avg_pool2d(conv2, kernel_size=8, stride=8)
        conv3d = F.avg_pool2d(conv3, kernel_size=4, stride=4)
        conv4d = F.avg_pool2d(conv4, kernel_size=2, stride=2)
        
        conv5 = self.outc1(torch.cat([conv1d,conv2d,conv3d,conv4d,conv5], dim=1))
        conv6 = self.outc2(conv5)
        core = self.outc3(conv6)    
        core = self.adapool(core)    # (bs, ch, kh, kw)
        
        bs, _, kh, kw = core.shape
        output = []
        for i, cur_im in enumerate(data_with_est):
            cur_im = cur_im.unsqueeze(0)                                            # cur_im (1, color, h, w)
            cur_im = F.pad(cur_im, [kw // 2, kw // 2, kh // 2, kh // 2], mode='replicate')
            cur_core = core[i].view(self.color, self.color, kh, kw)                 # cur_core (color, color, kh, kw)
            cur_out = F.conv2d(cur_im, cur_core)                                    # cur_out (1, color, h, w)
